PRO matchup_GlobColour, L3M_DDS=l3m_dds, L3B_DDS=l3b_dds, $
    SeaWiFS=seawifs, MODIS=modis, MERIS=meris, MERGED=merged, $
...
;SYNTAX:
;   matchup_GlobColour  [ [,/l3m_dds | ,/l3b_dds] [,/seawifs] [,/modis] [,/meris] [,/merged] [,/time_series] [CV=value] ]
;
;KEYWORDS:
...
;		merged -> run the matchup for merged products only (this will override /l3m_dds keyword as merged products are always in l3b format)
...
;	Jul 07 (added merged product matchup functionality)
;-
...
diffTime = (12.0/24.0)		;Maximum time difference in days between Satellite and in-situ measurements
TurbidFlag = 0						;To accept pixels flagged with TURBID set this variable to 1 else set 0
nPmap = 2									;number of pixels to extract from Mapped (1km) DDS adjacent to the match-up pixel. nPmap=2 will extract pixels from a (2n+1)x(2n+1) kernel
nPbin = 9									;number of pixels to extract from Binned (4km) DDS closest to the in-situ location.

if(keyword_set(merged)) then begin
	merged=1b
	sat_method=['AV','AVW','GSM']
	merge_hour=12			;time hour in gmt for merged product
	merge_minute=0		;time minute in gmt for merged product
endif	else begin
	sat_method=['SeaWiFS','MODIS','MERIS']
	merged=0b
endelse
;--Global parameters
;--------------------
...
;--------------------
;++Machine Dependent path separator
...
;GlobCOLOUR in-situ Field Names
 ...
 ;29->Flag       ;30->Campaign  ;31->Comments
    ...
    insituYearSDYtime=insituYearSDY+DOUBLE(insitu_data.Hour/24. + (insitu_data.Minute/1400.) )
...
if(KEYWORD_SET(l3b_dds) or merged) then begin
...
CD,filDir
if(KEYWORD_SET(l3b_dds) or merged) then MatchUpDirName='L3B_MatchUp' else MatchUpDirName='L3M_MatchUp'
FILE_MKDIR, MatchUpDirName

Sensor=4    ;Default is Find files for all sensors

;+#################12July2007#####################
if(keyword_set(merged)) then p=where(STREGEX(ddsFiles,'_AV-') NE -1, nSWF) else p=where(STREGEX(ddsFiles,'_SWF_DDS_') NE -1, nSWF)
if(nSWF GT 1) then ddsSWFFiles=ddsFiles(p)

if(keyword_set(merged)) then p=where(STREGEX(ddsFiles,'_AVW-') NE -1, nMOD) else p=where(STREGEX(ddsFiles,'_MOD_DDS_') NE -1, nMOD)
if(nMOD GT 1) then ddsMODFiles=ddsFiles(p)

if(keyword_set(merged)) then p=where(STREGEX(ddsFiles,'_GSM-') NE -1, nMER) else p=where(STREGEX(ddsFiles,'_MER_DDS_') NE -1, nMER)
...
;Filter DDS Files per Sensor
    ...
    if(nSWF GT 0) then begin
        openw,1,MatchUpDirName+separator+inDBname+'_v_'+sat_method(0)+'_DDS_Match_extract.csv'
        openw,2,MatchUpDirName+separator+inDBname+'_v_'+sat_method(0)+'_DDS_Match_average.csv'
...
if(KEYWORD_SET(modis)) then begin
    ...
    if(nMOD GT 0) then begin
        openw,4,MatchUpDirName+separator+inDBname+'_v_'+sat_method(1)+'_DDS_Match_extract.csv'
        openw,5,MatchUpDirName+separator+inDBname+'_v_'+sat_method(1)+'_DDS_Match_average.csv'
...
if(KEYWORD_SET(meris)) then begin
    ...
    if(nMER GT 0) then begin
        openw,7,MatchUpDirName+separator+inDBname+'_v_'+sat_method(2)+'_DDS_Match_extract.csv'
        openw,8,MatchUpDirName+separator+inDBname+'_v_'+sat_method(2)+'_DDS_Match_average.csv'
...
;[+1]. SeaWiFS
    ...
    if((Sensor EQ 1 or Sensor EQ 4) and nSWF GT 0) then begin
       for i=0,nSWF-1 do begin
        ...
        if(merged) then ddsHour=merge_hour else ddsHour=FIX(strmid(ddsFile,13,2))
        if(merged) then ddsMinute=merge_minute else ddsMinute=FIX(strmid(ddsFile,15,2))
        ddsTime=ddsHour + (ddsMinute/60.)

				ddsYearSDYtime=ddsYearSDY + double(ddsHour/24.+(ddsMinute/1440.))	;time including hours,minutes in yyyyddd.dd form

        ;day_match=where(insituYearSDY EQ ddsYearSDY, nr)
        day_match=where(ddsYearSDYtime GE (insituYearSDYtime-diffTime) and ddsYearSDYtime LE (insituYearSDYtime+diffTime), nr)
...
;Treat the match-up depending on the input_file type LAC=1.1km and GAC=4.4km effective resolution
            ...
            if( (total(strmatch(split_string,'MLAC')) GE 1) or (total(strmatch(split_string,'LAC')) GE 1) or (total(strmatch(split_string,'MER')) GE 1) ) then input_file_tag='LAC'
            if( (total(strmatch(split_string,'GAC')) GE 1) and (total(strmatch(split_string,'MLAC')) EQ 0) and (total(strmatch(split_string,'LAC')) EQ 0) and (total(strmatch(split_string,'MER')) EQ 0) ) then input_file_tag='GAC'
...
;[+1.1] L3 Binned DDS (All data are in 1-D array form)
              ...
              inDist=SQRT(offset[0]^2. + offset[1]^2.)
                ...
                p=NearestPoint(ddsLon, ddsLat, inLon(k), inLat(k), MaxDist=inDist, SR=inDist, nP=nPbin)
                nPix=n_elements(p)
...
;Initiate variable average, median, stdev for extracted pixels
                        ...
                        for pp=0, nPix-1 do begin
                          if(ddsCHLm(pp) GT 0.) then ddsCHLcv(pp)=ddsCHLs(pp)/ddsCHLm(pp)
                          if(ddsCHL2m(pp) GT 0.) then ddsCHL2cv(pp)=ddsCHL2s(pp)/ddsCHL2m(pp)
                          if(ddsKD490m(pp) GT 0.) then ddsKD490cv(pp)=ddsKD490s(pp)/ddsKD490m(pp)
                          if(ddsTSMm(pp) GT 0.) then ddsTSMcv(pp)=ddsTSMs(pp)/ddsTSMm(pp)
                          if(ddsCDMm(pp) GT 0.) then ddsCDMcv(pp)=ddsCDMs(pp)/ddsCDMm(pp)
                          if(ddsBBPm(pp) GT 0.) then ddsBBPcv(pp)=ddsBBPs(pp)/ddsBBPm(pp)
                          if(ddsT865m(pp) GT 0.) then ddsT865cv(pp)=ddsT865s(pp)/ddsT865m(pp)
                          if(ddsL412m(pp) GT 0.) then ddsL412cv(pp)=ddsL412s(pp)/ddsL412m(pp)
                          if(ddsL443m(pp) GT 0.) then ddsL443cv(pp)=ddsL443s(pp)/ddsL443m(pp)
                          if(ddsL490m(pp) GT 0.) then ddsL490cv(pp)=ddsL490s(pp)/ddsL490m(pp)
                          if(ddsL510m(pp) GT 0.) then ddsL510cv(pp)=ddsL510s(pp)/ddsL510m(pp)
                          if(ddsL531m(pp) GT 0.) then ddsL531cv(pp)=ddsL531s(pp)/ddsL531m(pp)
                          if(ddsL555m(pp) GT 0.) then ddsL555cv(pp)=ddsL555s(pp)/ddsL555m(pp)
                          if(ddsL620m(pp) GT 0.) then ddsL620cv(pp)=ddsL620s(pp)/ddsL620m(pp)
                          if(ddsL670m(pp) GT 0.) then ddsL670cv(pp)=ddsL670s(pp)/ddsL670m(pp)
                          if(ddsL681m(pp) GT 0.) then ddsL681cv(pp)=ddsL681s(pp)/ddsL681m(pp)
                          if(ddsL709m(pp) GT 0.) then ddsL709cv(pp)=ddsL709s(pp)/ddsL709m(pp)


                          flg=gcDDSFLAG2Name(ddsCHLf(pp), DEPTH=df, CLOUD=cf, TURBID=tf, VALID=vf)
                            if((vf EQ 1) and (tf LE TurbidFlag) and (cf LT 4) and (df LT 4) and (ddsCHLcv(pp) LT cvar)) then c1_msk(pp)=1
                          flg=gcDDSFLAG2Name(ddsCHL2f(pp), DEPTH=df, CLOUD=cf, TURBID=tf, VALID=vf)
                            if((vf EQ 1) and (tf LE TurbidFlag) and (cf LT 4) and (df LE 4) and (ddsCHL2cv(pp) LT cvar)) then c2_msk(pp)=1
                          flg=gcDDSFLAG2Name(ddsKD490f(pp), DEPTH=df, CLOUD=cf, TURBID=tf, VALID=vf)
                            if((vf EQ 1) and (tf LE TurbidFlag) and (cf LT 4) and (df LT 4) and (ddsKD490cv(pp) LT cvar)) then kd_msk(pp)=1
...
;[+1.2] L3 Mapped DDS (All data are in 2-D array form; Lat/Lon are 1-D arrays
              ...
              inDist=SQRT(offset[0]^2. + offset[1]^2.)
                ...
                p=NearestPoint(subLon, subLat, inLon(k), inLat(k), MaxDist=inDist, nP=nPmap)
...
;Initiate variable average, median, stdev for extracted pixels
                        ...
                        for pp=0, nPix-1 do begin
                          flg=gcDDSFLAG2Name(ddsCHLf(pp), DEPTH=df, CLOUD=cf, TURBID=tf, VALID=vf)
                            if((vf EQ 1) and (tf LE TurbidFlag) and (cf LT 4) and (df LT 4)) then c1_msk(pp)=1
                          flg=gcDDSFLAG2Name(ddsCHL2f(pp), DEPTH=df, CLOUD=cf, TURBID=tf, VALID=vf)
                            if((vf EQ 1) and (tf LE TurbidFlag) and (cf LT 4) and (df LE 4)) then c2_msk(pp)=1   ;(df LT 4)
                          flg=gcDDSFLAG2Name(ddsKD490f(pp), DEPTH=df, CLOUD=cf, TURBID=tf, VALID=vf)
                            if((vf EQ 1) and (tf LE TurbidFlag) and (cf LT 4) and (df LT 4)) then kd_msk(pp)=1
                          flg=gcDDSFLAG2Name(ddsTSMf(pp), DEPTH=df, CLOUD=cf, TURBID=tf, VALID=vf)
...
;[+2]. MODIS
    ...
    if((Sensor EQ 2 or Sensor EQ 4) and nMOD GT 0) then begin
       for i=0,nMOD-1 do begin
        ...
        ;day_match=where(insituYearSDY EQ ddsYearSDY, nr)
        day_match=where(ddsYearSDYtime GE (insituYearSDYtime-diffTime) and ddsYearSDYtime LE (insituYearSDYtime+diffTime), nr)        ;For time-series type in-situ pick only the closest in-situ record
        if(keyword_set(time_series)) then begin
            if(nr GT 1) then begin
            tDiff=abs(ddsTime - insituTime(day_match))
            mintDiff=where(tDiff EQ min(tDiff))
...
;print,'CHL1 FLAG : ',gcDDSFlag2Name(ddsCHLf(pp)), ddsCHLs(pp), ddsCHLm(pp), ddsCHLcv(pp), format='(20(A),3(F,2x))'
                          flg=gcDDSFLAG2Name(ddsCHLf(pp), DEPTH=df, CLOUD=cf, TURBID=tf, VALID=vf)
                            if((vf EQ 1) and (tf LE TurbidFlag) and (cf LT 4) and (df LT 4) and (ddsCHLcv(pp) LT cvar)) then c1_msk(pp)=1
                          flg=gcDDSFLAG2Name(ddsCHL2f(pp), DEPTH=df, CLOUD=cf, TURBID=tf, VALID=vf)
                            if((vf EQ 1) and (tf LE TurbidFlag) and (cf LT 4) and (df LE 4) and (ddsCHL2cv(pp) LT cvar)) then c2_msk(pp)=1
