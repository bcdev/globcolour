PRO matchup_Stat_GlobColour, Max_Time_Diff=mtd, $
		CATCH_DUPLICATES=catch_duplicates, LIST_FILENAME=list_filename, SHOW=show
...
;SYNTAX:
; matchup_Stat_GlobColour [,Max_Time_Diff=value] [,/list_filename] [,/catch_duplicates] [,/show]
;INPUT FILE:
; '....DDS_Match_average.csv'
;OUTPUTS:
; '..._DDS_Match_summary1.csv'		[using standard average values]
; '..._DDS_Match_summary2.csv'		[using filtered average values]
; '..._DDS_Match_average.eps'			[using both standard and fileterd average data]
;
;KEYWORDS:
; Max_Time_Diff -> Maximum allowed time difference between in-situ and satellite observation (in hours); default is 12 hrs
; catch_duplicates -> Flags duplicate entried in the match-up file based on inID
;	list_filename -> Writes the list of match-up DDS filenames for each variables
; show -> shows the stat plots (spawning with gsview; change according to your local eps viewer)

;	$Id: matchup_Stat_GlobColour.pro,v 2.07.2 01/05/2007 11:30:13 yaswant Exp $
; matchup_Stat_GlobColour.pro	Yaswant Pradhan	University of Plymouth
;	Last modification: Mar 07, Apr'07 show keyword is not mandatory to write the eps file (yp)
;	May'07 list_file kyeword added (yp)
;-
...
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
;Change these parameters to suite your satisfaction
 if( ~KEYWORD_SET(mtd) ) then mtd=12.0	;Maximum absoulte time difference in hours
 if( KEYWORD_SET(catch_duplicates) ) then noDup=1B else noDup=0B	;Only when you take the scattered matchup like OBPG/NOMAD
 cvar = 0.2   ;Coefficient of variation is already used in the calculation of FILTERED_AVERAGE from L3m files
 if( KEYWORD_SET(list_filename) ) then list=1b else list=0b	;Lists the DDS files used in the matchup

;Number of Good pixels used in the averaging (input-type dependent)
	ngp_l3m_lac = 13.	;NGP for GlobColour level-3 mapped file for which the input L2 file was a (M)LAC file (should be 13 or more)
	ngp_l3m_gac = 2.	;NGP for GlobColour level-3 mapped file for which the input L2 file was a GAC file (should be at least 2)
	ngp_l3b = 5.	 		;NGP for GlobColour level-3 binned file for which the input L2 file was either GAC or (M)LAC file (should be at least 2)
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  for i=0, records-1 do begin
    wh=where(strcmp(data.inID[i], data.inID),cnt)
      ...
      if(cnt GT 1) then begin
        ...
        r=where(tot EQ max(tot,/NaN), nr, complement=x, ncomplement=nx)

        ;Flag out all smaller tot
        ...
        endif;if(nx GT 0) then begin

        ;If there are more than one max tot then compute the distance from in-situ point for all replica
        if(nr GT 1) then begin
          ...
          endfor;for m=0,cnt-1 do begin
          r2=where(dist EQ min(dist(r),/NaN), nr2, complement=x, ncomplement=nx)

          ;Flag all points satifying ~minimum distance criteria
          if(nx GT 0) then begin
          	for k=0,n_tags(data)-1 do data.(k)[wh(x)]=!Values.F_NaN
            data.inTime[wh(x)]='00000000T000000Z'   ;To avoid the type conversion error message
            data.ddsTime[wh(x)]='00000000T000000Z'
					...
					;If there are more than one point satisfy the minimum distance criterion, Keep the first one only
          if(nr2 GT 1) then begin
          	for k=0,n_tags(data)-1 do data.(k)[wh(r2[1:*])]=!Values.F_NaN
          	data.inTime[wh(r2[1:*])]='00000000T000000Z' ;To avoid the type conversion error message
          	data.ddsTime[wh(r2[1:*])]='00000000T000000Z'
          endif;if(nr2 GT 1) then begin
        endif;if(nr GT 1) then begin

      endif;if(cnt GT 1) then begin

  endfor;for i=0, records-1 do begin

endif;if(noDup) then begin
...
;----------------------------------------------
;+	Build Number of Good Pixels variable
 	file_type=strmid( data.ddsFilename(p),0,3 )
 	file_tag=data.Input_File_Tag(p)
 	np=intarr(cnt1)
 	for c=0,cnt1-1 do begin
 		if( strcmp(file_type(c),'L3b',/fold_case) ) then np(c)=ngp_l3b else $
 		if( strcmp(file_type(c),'L3m',/fold_case) ) then begin
 			if( strcmp(file_tag(c),'LAC',/fold_case) ) then np(c)=ngp_l3m_lac else $
 			if( strcmp(file_tag(c),'GAC',/fold_case) ) then np(c)=ngp_l3m_gac
 		endif
 	endfor
;-	Build Number of Good Pixels variable
;----------------------------------------------
;print,'NPPPPPPPPP-->',np
 ...
 ;+1.1 With standard average data
    ...
    pp=where((inChl GT 0.) and (ddsCHL1_a GT 0) and ((ddsCHL1_s/ddsCHL1_a) LT cvar) and ((data.ddsChl1_N(p)-np) GE 0), nCHL1_a)
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_Chl1_matchup_list1.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inChl,'+sen_name+'_Chl1,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inChl1)-1 do begin
       		;printf,lun,data.ddsFilename(p(pp))
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inChl1(mr),ddsCHL1_a(mr),data.ddsCHL1_N(p(pp(mr))),data.ddsCHL1_std(p(pp(mr))),ddsCHL1_a(mr)/inChl1(mr),abs(ddsCHL1_a(mr)-inChl1(mr))/inChl1(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nCHL1_a GT 2) then begin
;-1.1 With standard average data
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_Chl1_matchup_list2.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inChl,'+sen_name+'_Chl1,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inChl12)-1 do begin
       		;printf,lun,data.ddsFilename(p(pp))
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inChl12(mr),ddsCHL1_a2(mr),data.ddsCHL1_N2(p(pp(mr))),data.ddsCHL1_std2(p(pp(mr))),ddsCHL1_a2(mr)/inChl12(mr),abs(ddsCHL1_a2(mr)-inChl12(mr))/inChl12(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nCHL1_a2 GT 2) then begin
;-1.2 With filtered average data
;-1. CHL1
...
;+2. CHL2
 ;+2.1 With standard average data
    ...
    pp=where((inChl GT 0.) and (ddsCHL2_a GT 0) and ((ddsCHL2_s/ddsCHL2_a) LT cvar) and ((data.ddsChl2_N(p)-np) GE 0), nCHL2_a)
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_Chl2_matchup_list1.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inChl,'+sen_name+'_Chl2,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inChl2)-1 do begin
       		;printf,lun,data.ddsFilename(p(pp))
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inChl2(mr),ddsCHL2_a(mr),data.ddsCHL2_N(p(pp(mr))),data.ddsCHL2_std(p(pp(mr))),ddsCHL2_a(mr)/inChl2(mr),abs(ddsCHL2_a(mr)-inChl2(mr))/inChl2(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nCHL2_a GT 2) then begin
;-2.1 With standard average data
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_Chl2_matchup_list2.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inChl,'+sen_name+'_Chl2,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inChl22)-1 do begin
       		;printf,lun,data.ddsFilename(p(pp))
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inChl22(mr),ddsCHL2_a2(mr),data.ddsCHL2_N2(p(pp(mr))),data.ddsCHL2_std2(p(pp(mr))),ddsCHL2_a2(mr)/inChl22(mr),abs(ddsCHL2_a2(mr)-inChl22(mr))/inChl22(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nCHL2_a2 GT 2) then begin
;-2.2 With filtered average data
;-2. CHL2
...
;+3. KD490
 ;+3.1 With standard average data
    ...
    pp=where((inK490 GT 0.) and (ddsK490_a GT 0) and ((ddsK490_s/ddsK490_a) LT cvar) and ((data.ddsK490_N(p)-np) GE 0), nK490_a)
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_K490_matchup_list1.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inK490,'+sen_name+'_K490,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inK4901)-1 do begin
       		;printf,lun,data.ddsFilename(p(pp))
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inK4901(mr),ddsK490_a(mr),data.ddsK490_N(p(pp(mr))),data.ddsK490_std(p(pp(mr))),ddsK490_a(mr)/inK4901(mr),abs(ddsK490_a(mr)-inK4901(mr))/inK4901(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nK490_a GT 2) then begin
;-3.1 With standard average data
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_K490_matchup_list2.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inK490,'+sen_name+'_K490,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inK4902)-1 do begin
       		;printf,lun,data.ddsFilename(p(pp))
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inK4902(mr),ddsK490_a2(mr),data.ddsK490_N2(p(pp(mr))),data.ddsK490_std2(p(pp(mr))),ddsK490_a2(mr)/inK4902(mr),abs(ddsK490_a2(mr)-inK4902(mr))/inK4902(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nK490_a2 GT 2) then begin
;-3.2 With filtered average data
;-3. KD490
...
;+4. CDM
 ;+4.1 With standard average data
    ...
    pp=where((inaCDM GT 0.) and (ddsaCDM_a GT 0) and ((ddsaCDM_s/ddsaCDM_a) LT cvar) and ((data.ddsaCDM_N(p)-np) GE 0), nCDM_a)
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_CDM_matchup_list1.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inCDM443,'+sen_name+'_CDM443,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inaCDM1)-1 do begin
       		;printf,lun,data.ddsFilename(p(pp))
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inaCDM1(mr),ddsaCDM_a(mr),data.ddsaCDM_N(p(pp(mr))),data.ddsaCDM_std(p(pp(mr))),ddsaCDM_a(mr)/inaCDM1(mr),abs(ddsaCDM_a(mr)-inaCDM1(mr))/inaCDM1(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nCDM_a GT 2) then begin
;-4.1 With standard average data
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_CDM_matchup_list2.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inCDM443,'+sen_name+'_CDM443,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inaCDM2)-1 do begin
       		;printf,lun,data.ddsFilename(p(pp))
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inaCDM2(mr),ddsaCDM_a2(mr),data.ddsaCDM_N2(p(pp(mr))),data.ddsaCDM_std2(p(pp(mr))),ddsaCDM_a2(mr)/inaCDM2(mr),abs(ddsaCDM_a2(mr)-inaCDM2(mr))/inaCDM2(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nCDM_a2 GT 2) then begin
;-4.2 With filtered average data
;-4. CDM
...
;+5. BBP
 ;+5.1 With standard average data
    ...
    pp=where((inBBP GT 0.) and (ddsBBP_a GT 0) and ((ddsBBP_s/ddsBBP_a) LT cvar) and ((data.ddsBBP_N(p)-np) GE 0), nBBP_a)
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_BBP_matchup_list1.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inBBP443,'+sen_name+'_BBP443,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inBBP1)-1 do begin
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inBBP1(mr),ddsBBP_a(mr),data.ddsBBP_N(p(pp(mr))),data.ddsBBP_std(p(pp(mr))),ddsBBP_a(mr)/inaBBP1(mr),abs(ddsBBP_a(mr)-inBBP1(mr))/inBBP1(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nBBP_a GT 2) then begin
;-5.1 With standard average data
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_BBP_matchup_list2.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inBBP443,'+sen_name+'_BBP443,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inBBP2)-1 do begin
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inBBP2(mr),ddsBBP_a2(mr),data.ddsBBP_N2(p(pp(mr))),data.ddsBBP_std2(p(pp(mr))),ddsBBP_a2(mr)/inaBBP2(mr),abs(ddsBBP_a2(mr)-inBBP2(mr))/inBBP2(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nBBP_a2 GT 2) then begin
;-5.2 With filtered average data
;-5. BBP
...
;+6. TSM
 ;+6.1 With standard average data
    ...
    pp=where((inTSM GT 0.) and (ddsTSM_a GT 0) and ((ddsTSM_s/ddsTSM_a) LT cvar) and ((data.ddsTSM_N(p)-np) GE 0), nTSM_a)
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_TSM_matchup_list1.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inTSM,'+sen_name+'_TSM,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inTSM1)-1 do begin
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inTSM1(mr),ddsTSM_a(mr),data.ddsTSM_N(p(pp(mr))),data.ddsTSM_std(p(pp(mr))),ddsTSM_a(mr)/inaTSM1(mr),abs(ddsTSM_a(mr)-inTSM1(mr))/inTSM1(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nTSM_a GT 2) then begin
;-6.1 With standard average data
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_TSM_matchup_list2.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inTSM,'+sen_name+'_TSM,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inTSM1)-1 do begin
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inTSM2(mr),ddsTSM_a2(mr),data.ddsTSM_N2(p(pp(mr))),data.ddsTSM_std2(p(pp(mr))),ddsTSM_a2(mr)/inaTSM2(mr),abs(ddsTSM_a2(mr)-inTSM2(mr))/inTSM2(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nTSM_a2 GT 2) then begin
;-6.2 With filtered average data
;-6. TSM
...
;+7. T865
 ;+7.1 With standard average data
    ...
    pp=where((inT865 GT 0.) and (ddsT865_a GT 0) and ((ddsT865_s/ddsT865_a) LT cvar) and ((data.ddsT865_N(p)-np) GE 0), nT865_a)
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_T865_matchup_list1.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inT865,'+sen_name+'_T865,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inT8651)-1 do begin
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inT8651(mr),ddsT865_a(mr),data.ddsT865_N(p(pp(mr))),data.ddsT865_std(p(pp(mr))),ddsT865_a(mr)/inT8651(mr),abs(ddsT865_a(mr)-inT8651(mr))/inT8651(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nT865_a GT 2) then begin
;-7.1 With standard average data
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_T865_matchup_list2.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inT865,'+sen_name+'_T865,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inT8652)-1 do begin
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inT8652(mr),ddsT865_a2(mr),data.ddsT865_N2(p(pp(mr))),data.ddsT865_std2(p(pp(mr))),ddsT865_a2(mr)/inT8652(mr),abs(ddsT865_a2(mr)-inT8652(mr))/inT8652(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nT865_a2 GT 2) then begin
;-7.2 With filtered average data
;-7. T865
...
;+8. L412
 ;+8.1 With standard average data
    ...
    pp=where((inL412 GT 0.) and (ddsL412_a GT 0) and ((ddsL412_s/ddsL412_a) LT cvar) and ((data.ddsL412_N(p)-np) GE 0), nL412_a)
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_L412_matchup_list1.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inL412,'+sen_name+'_L412,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inL4121)-1 do begin
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inL4121(mr),ddsL412_a(mr),data.ddsL412_N(p(pp(mr))),data.ddsL412_std(p(pp(mr))),ddsL412_a(mr)/inL4121(mr),abs(ddsL412_a(mr)-inL4121(mr))/inL4121(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nL412_a GT 2) then begin
;-8.1 With standard average data
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_L412_matchup_list2.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inL412,'+sen_name+'_L412,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inL4122)-1 do begin
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inL4122(mr),ddsL412_a2(mr),data.ddsL412_N2(p(pp(mr))),data.ddsL412_std2(p(pp(mr))),ddsL412_a2(mr)/inL4122(mr),abs(ddsL412_a2(mr)-inL4122(mr))/inL4122(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nL412_a2 GT 2) then begin
;-8.2 With filtered average data
;-8. L412
...
;+9. L443
 ;+9.1 With standard average data
    ...
    pp=where((inL443 GT 0.) and (ddsL443_a GT 0) and ((ddsL443_s/ddsL443_a) LT cvar) and ((data.ddsL443_N(p)-np) GE 0), nL443_a)
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_L443_matchup_list1.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inL443,'+sen_name+'_L443,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inL4431)-1 do begin
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inL4431(mr),ddsL443_a(mr),data.ddsL443_N(p(pp(mr))),data.ddsL443_std(p(pp(mr))),ddsL443_a(mr)/inL4431(mr),abs(ddsL443_a(mr)-inL4431(mr))/inL4431(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nL443_a GT 2) then begin
;-9.1 With standard average data
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_L443_matchup_list2.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inL443,'+sen_name+'_L443,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inL4432)-1 do begin
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inL4432(mr),ddsL443_a2(mr),data.ddsL443_N2(p(pp(mr))),data.ddsL443_std2(p(pp(mr))),ddsL443_a2(mr)/inL4432(mr),abs(ddsL443_a2(mr)-inL4432(mr))/inL4432(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nL443_a2 GT 2) then begin
;-9.2 With filtered average data
;-9. L443
...
;+10. L490
 ;+10.1 With standard average data
    ...
    pp=where((inL490 GT 0.) and (ddsL490_a GT 0) and ((ddsL490_s/ddsL490_a) LT cvar) and ((data.ddsL490_N(p)-np) GE 0), nL490_a)
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_L490_matchup_list1.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inL490,'+sen_name+'_L490,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inL4901)-1 do begin
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inL4901(mr),ddsL490_a(mr),data.ddsL490_N(p(pp(mr))),data.ddsL490_std(p(pp(mr))),ddsL490_a(mr)/inL4901(mr),abs(ddsL490_a(mr)-inL4901(mr))/inL4901(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nL490_a GT 2) then begin
;-10.1 With standard average data
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_L490_matchup_list2.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inL490,'+sen_name+'_L490,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inL4902)-1 do begin
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inL4902(mr),ddsL490_a2(mr),data.ddsL490_N2(p(pp(mr))),data.ddsL490_std2(p(pp(mr))),ddsL490_a2(mr)/inL4902(mr),abs(ddsL490_a2(mr)-inL4902(mr))/inL4902(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nL490_a2 GT 2) then begin
;-10.2 With filtered average data
;-10. L490
...
;+11. L510
 ;+11.1 With standard average data
    ...
    pp=where((inL510 GT 0.) and (ddsL510_a GT 0) and ((ddsL510_s/ddsL510_a) LT cvar) and ((data.ddsL510_N(p)-np) GE 0), nL510_a)
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_L510_matchup_list1.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inL510,'+sen_name+'_L510,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inL5101)-1 do begin
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inL5101(mr),ddsL510_a(mr),data.ddsL510_N(p(pp(mr))),data.ddsL510_std(p(pp(mr))),ddsL510_a(mr)/inL5101(mr),abs(ddsL510_a(mr)-inL5101(mr))/inL5101(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nL510_a GT 2) then begin
;-11.1 With standard average data
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_L510_matchup_list2.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inL510,'+sen_name+'_L510,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inL5102)-1 do begin
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inL5102(mr),ddsL510_a2(mr),data.ddsL510_N2(p(pp(mr))),data.ddsL510_std2(p(pp(mr))),ddsL510_a2(mr)/inL5102(mr),abs(ddsL510_a2(mr)-inL5102(mr))/inL5102(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nL510_a2 GT 2) then begin
;-11.2 With filtered average data
;-11. L510
...
;+12. L531
 ;+12.1 With standard average data
    ...
    pp=where((inL531 GT 0.) and (ddsL531_a GT 0) and ((ddsL531_s/ddsL531_a) LT cvar) and ((data.ddsL531_N(p)-np) GE 0), nL531_a)
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_L531_matchup_list1.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inL531,'+sen_name+'_L531,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inL5311)-1 do begin
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inL5311(mr),ddsL531_a(mr),data.ddsL531_N(p(pp(mr))),data.ddsL531_std(p(pp(mr))),ddsL531_a(mr)/inL5311(mr),abs(ddsL531_a(mr)-inL5311(mr))/inL5311(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nL531_a GT 2) then begin
;-12.1 With standard average data
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_L531_matchup_list2.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inL531,'+sen_name+'_L531,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inL5312)-1 do begin
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inL5312(mr),ddsL531_a2(mr),data.ddsL531_N2(p(pp(mr))),data.ddsL531_std2(p(pp(mr))),ddsL531_a2(mr)/inL5312(mr),abs(ddsL531_a2(mr)-inL5312(mr))/inL5312(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nL531_a2 GT 2) then begin
;-12.2 With filtered average data
;-12. L531
...
;+13. L555
 ;+13.1 With standard average data
    ...
    pp=where((inL555 GT 0.) and (ddsL555_a GT 0) and ((ddsL555_s/ddsL555_a) LT cvar) and ((data.ddsL555_N(p)-np) GE 0), nL555_a)
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_L555_matchup_list1.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inL555,'+sen_name+'_L555,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inL5551)-1 do begin
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inL5551(mr),ddsL555_a(mr),data.ddsL555_N(p(pp(mr))),data.ddsL555_std(p(pp(mr))),ddsL555_a(mr)/inL5551(mr),abs(ddsL555_a(mr)-inL5551(mr))/inL5551(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nL555_a GT 2) then begin
;-13.1 With standard average data
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_L555_matchup_list2.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inL555,'+sen_name+'_L555,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inL5552)-1 do begin
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inL5552(mr),ddsL555_a2(mr),data.ddsL555_N2(p(pp(mr))),data.ddsL555_std2(p(pp(mr))),ddsL555_a2(mr)/inL5552(mr),abs(ddsL555_a2(mr)-inL5552(mr))/inL5552(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nL555_a2 GT 2) then begin
;-13.2 With filtered average data
;-13. L555
...
;+14. L620
 ;+14.1 With standard average data
    ...
    pp=where((inL620 GT 0.) and (ddsL620_a GT 0) and ((ddsL620_s/ddsL620_a) LT cvar) and ((data.ddsL620_N(p)-np) GE 0), nL620_a)
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_L620_matchup_list1.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inL620,'+sen_name+'_L620,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inL6201)-1 do begin
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inL6201(mr),ddsL620_a(mr),data.ddsL620_N(p(pp(mr))),data.ddsL620_std(p(pp(mr))),ddsL620_a(mr)/inL6201(mr),abs(ddsL620_a(mr)-inL6201(mr))/inL6201(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nL620_a GT 2) then begin
;-14.1 With standard average data
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_L620_matchup_list2.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inL620,'+sen_name+'_L620,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inL6202)-1 do begin
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inL6202(mr),ddsL620_a2(mr),data.ddsL620_N2(p(pp(mr))),data.ddsL620_std2(p(pp(mr))),ddsL620_a2(mr)/inL6202(mr),abs(ddsL620_a2(mr)-inL6202(mr))/inL6202(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nL620_a2 GT 2) then begin
;-14.2 With filtered average data
;-14. L620
...
;+15. L670
 ;+15.1 With standard average data
    ...
    pp=where((inL670 GT 0.) and (ddsL670_a GT 0) and ((ddsL670_s/ddsL670_a) LT cvar) and ((data.ddsL670_N(p)-np) GE 0), nL670_a)
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_L670_matchup_list1.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inL670,'+sen_name+'_L670,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inL6701)-1 do begin
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inL6701(mr),ddsL670_a(mr),data.ddsL670_N(p(pp(mr))),data.ddsL670_std(p(pp(mr))),ddsL670_a(mr)/inL6701(mr),abs(ddsL670_a(mr)-inL6701(mr))/inL6701(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nL670_a GT 2) then begin
;-15.1 With standard average data
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_L670_matchup_list2.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inL670,'+sen_name+'_L670,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inL6702)-1 do begin
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inL6702(mr),ddsL670_a2(mr),data.ddsL670_N2(p(pp(mr))),data.ddsL670_std2(p(pp(mr))),ddsL670_a2(mr)/inL6702(mr),abs(ddsL670_a2(mr)-inL6702(mr))/inL6702(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nL670_a2 GT 2) then begin
;-15.2 With filtered average data
;-15. L670
...
;+16. L681
 ;+16.1 With standard average data
    ...
    pp=where((inL681 GT 0.) and (ddsL681_a GT 0) and ((ddsL681_s/ddsL681_a) LT cvar) and ((data.ddsL681_N(p)-np) GE 0), nL681_a)
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_L681_matchup_list1.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inL681,'+sen_name+'_L681,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inL6811)-1 do begin
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inL6811(mr),ddsL681_a(mr),data.ddsL681_N(p(pp(mr))),data.ddsL681_std(p(pp(mr))),ddsL681_a(mr)/inL6811(mr),abs(ddsL681_a(mr)-inL6811(mr))/inL6811(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nL681_a GT 2) then begin
;-16.1 With standard average data
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_L681_matchup_list2.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inL681,'+sen_name+'_L681,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inL6812)-1 do begin
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inL6812(mr),ddsL681_a2(mr),data.ddsL681_N2(p(pp(mr))),data.ddsL681_std2(p(pp(mr))),ddsL681_a2(mr)/inL6812(mr),abs(ddsL681_a2(mr)-inL6812(mr))/inL6812(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nL681_a2 GT 2) then begin
;-16.2 With filtered average data
;-16. L681
...
;+17. L709
 ;+17.1 With standard average data
    ...
    pp=where((inL709 GT 0.) and (ddsL709_a GT 0) and ((ddsL709_s/ddsL709_a) LT cvar) and ((data.ddsL709_N(p)-np) GE 0), nL709_a)
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_L709_matchup_list1.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inL709,'+sen_name+'_L709,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inL7091)-1 do begin
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inL7091(mr),ddsL709_a(mr),data.ddsL709_N(p(pp(mr))),data.ddsL709_std(p(pp(mr))),ddsL709_a(mr)/inL7091(mr),abs(ddsL709_a(mr)-inL7091(mr))/inL7091(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nL709_a GT 2) then begin
;-17.1 With standard average data
...
;~~~~~
       if(list) then begin
       	openw,lun,sen_name+'_'+indb_name+'_L709_matchup_list2.csv',/get_lun
       	printf,lun,'inID,inTime,satTime,Lon,Lat,inL709,'+sen_name+'_L709,NPix,PixStd,Ratio,%Diff,Filename'
       	for mr=0,n_elements(inL7092)-1 do begin
       		printf,lun, data.inID(p(pp(mr))),data.inTime(p(pp(mr))),data.ddsTime(p(pp(mr))),data.inLon(p(pp(mr))),data.inLat(p(pp(mr))),$
       		inL7092(mr),ddsL709_a2(mr),data.ddsL709_N2(p(pp(mr))),data.ddsL709_std2(p(pp(mr))),ddsL709_a2(mr)/inL7092(mr),abs(ddsL709_a2(mr)-inL7092(mr))/inL7092(mr)*100.,$
       		data.ddsFilename(p(pp(mr))),format='(3(A,","),8(F10.5,","),A)'
       	endfor
       	free_lun,lun
       endif	;if(list) then begin
;~~~~~
    endif   ;if(nL709_a2 GT 2) then begin
;-17.2 With filtered average data
;-17. L709
...
;--------
 ;+1.CHL1
    ...
    if(nCHL1_a2 GE 2) then begin
      OPLOT,inChl12,ddsCHL1_a2,psym=SYMCAT(16),SYMSIZE=.6,COLOR=4
      ERRPLOT,inChl12,ddsCHL1_a2-ddsCHL1_s2,ddsCHL1_a2+ddsCHL1_s2
      OPLOT,xr,yr
