...
;
; PURPOSE:
;   Read GlobColour Global netCDF file into structure variable
;
; CATEGORY:
;   All levels of processing
;
; CALLING SEQUENCE:
;   read_GCnetCDF, filename, data, attributes, status [[,/SUBSET] [,COUNT=value] [,OFFSET=value]]
;
; INPUTS:
...
;   04/13/2007    Yaswant Pradhan addedd SUBSET (offset, count) keywords for GlobColour compatibility
;
; $Log: read_netcdf.pro,v $
; Revision 6.2  2007/04/13 17:17:40  yaswant
...
;
;
;idver='$Id: read_netcdf.pro,v 6.1 2004/07/07 22:34:39 yaswant Exp $'
;
;+

pro read_GCnetCDF, filename, data, attributes, status, $
                 SUBSET=subset, COUNT=cnt, OFFSET=oset
...
if (KEYWORD_SET(SUBSET)) then begin
    if(~KEYWORD_SET(oset)) then oset=0
    if(~KEYWORD_SET(cnt)) then cnt=2
endif
...
if (n_params(0) lt 1) then begin
    print, 'USAGE: read_GCnetCDF, filename, data, attributes, status [[,/SUBSET] [,COUNT=value] [,OFFSET=value]]'
    return
endif
...
;
;   option switch to import all data (due to size difference) when subset keyword set
;
opt=bytarr(finq.nvars)
...
for k=0, finq.nvars-1 do begin
    ...
    if ( keyword_set(subset) ) then begin
        if( n_elements(theData) GE oset+cnt ) then begin
            opt[k]=1b
            var_inq[k].ptr = PTR_NEW( theData[oset:oset+cnt-1] )
        endif else var_inq[k].ptr = PTR_NEW( theData )
    endif else var_inq[k].ptr = PTR_NEW( theData )    ;+yaswant
endfor
;stop
if (debug_mode gt 0) then begin
    print, ' '
...
for k=0, finq.nvars-1 do begin
    case var_inq[k].nest_level of
       0:  begin
         if (keyword_set(subset)) then begin
            if( opt[k] ) then NCDF_VARGET, fid, k, value, COUNT=cnt, OFFSET=oset $
            else NCDF_VARGET, fid, k, value
         endif else NCDF_VARGET, fid, k, value       ;+yaswant
